...
int main (int argc, char ** argv) {
  ...
  checkCudaErrors (cudaMalloc (&dev_x, byteSize_x));
  checkCudaErrors (cudaMalloc (&dev_y_computed,byteSize_y_reference));

  // copy input to dev_x, dev_y_computed
  checkCudaErrors (cudaMemcpy (dev_x, x.data(), byteSize_x, cudaMemcpyHostToDevice));
  checkCudaErrors (cudaMemcpy (dev_y_computed,y_reference.data(), byteSize_y_reference, cudaMemcpyHostToDevice));
  ...
  for(int i = 0; i < 1000; i++){
    saxpy_ (&n, &alpha, x.data(), &incx, y_reference.data(), &incy);
  }
  fortranTimer.stop();
  ...
  // execute our saxpy
  for(int i = 0; i < 1000; i++){
    saxpy (n, alpha, dev_x, inx, dev_y_computed, incy );
  }
  saxpyTimer.stop();  
  ...
  std::vector<float> y_computed (y_reference.size(), 0.0f);
  checkCudaErrors (cudaMemcpy (y_computed.data(), dev_y_computed, byteSize_y_reference, cudaMemcpyDeviceToHost));
  double relerr = relative_error_l2 (n, y_reference.data(), incy, t_computed.data(), incy);
