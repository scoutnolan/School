...
void compute_temp() {
    ...
    if(up_pr > numprocs - 1) up_flag = false;
    if(down_pr < 0) down_flag = false;

    int size1, size2, buffer_size; //declare sizes

    MPI_Pack_size(total_cols,MPI_DOUBLE, MPI_COMM_WORLD, &size1);
    MPI_Pack_size(total_cols,MPI_DOUBLE, MPI_COMM_WORLD, &size2);

    buffer_size = size1 + size2 + 2 * MPI_BSEND_OVERHEAD;
    ...
    for (int i=0;i<num_iterations;i++) {
        if (rank%2==0) { // even numbered processes
            if(up_flag){
                MPI_Bsend(&temp[active_rows_on_proc*total_cols],total_cols,
            ...
            if (rank>0) {
                if(down_flag){
                    MPI_Bsend(&temp[total_cols],total_cols,MPI_DOUBLE,
            ...
            if (rank < numprocs-1) {
                if(up_flag){
                    MPI_Bsend(&temp[active_rows_on_proc*total_cols],
...
int main (int argc, char *argv[]){
    ...
    // is to process 
    //active_rows_on_proc = n/numprocs + 1; //+1 so that for odd number of processes

    //divide the number of rows each process has depending on the rank

    if(rank < n % numprocs) {
        active_rows_on_proc = n/numprocs + 1;
    }
