...
void scatter(double *numbers,double *group,int num_size, int root, int rank,int numprocs)
{
   ...
   // const int number_elements_per_section = ceil(double(num_size) / double(numprocs));
   int number_elements_per_section = 0;
   if(rank < num_size % numprocs) number_elements_per_section = num_size/numprocs + 1;
      else number_elements_per_section = num_size / numprocs;
   ...
   if (rank==root) {
      ...
      for(int mpiproc = 0; mpiproc < numprocs; mpiproc++) {
         ...
         if(mpiproc < num_size % numprocs) number_elements_per_section = num_size/numprocs + 1;
            else number_elements_per_section = num_size / numprocs;         
         ...
         // the next MPI process
            ...
            MPI::COMM_WORLD.Send(&numbers[begin_element],
               number_elements_per_section,  // Count
...
int main(int argc, char *argv[]) {
   ...
   // const int local_list_size = ceil(double(list_size) / double(numprocs));
   int local_list_size = 0;
   if(rank < list_size % numprocs) local_list_size = list_size/numprocs + 1;
      else local_list_size = list_size / numprocs;
