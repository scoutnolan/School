        ...
        if (rank%2==0) { // even numbered processes
            if(up_flag){
                MPI_Isend(&temp[active_rows_on_proc*total_cols],total_cols,
                        MPI_DOUBLE,up_pr,123,MPI_COMM_WORLD, &req1);
            ...
            if (rank>0) {
                if(down_flag){
                    MPI_Isend(&temp[total_cols],total_cols,MPI_DOUBLE,
                            down_pr,123,MPI_COMM_WORLD, &req2);
            ...
            int flag1;
            if(rank != numprocs - 1){
                do MPI_Test(&req1, &flag1, &status); while(flag1 == 0);
            }
            ...
            if (rank < numprocs-1) {
                if(up_flag){
                    MPI_Recv(&temp[(active_rows_on_proc+1)*total_cols],
                            ...
                            MPI_ANY_TAG,MPI_COMM_WORLD,&status);
                    MPI_Isend(&temp[active_rows_on_proc*total_cols],
                            total_cols,MPI_DOUBLE,up_pr,
                            123,MPI_COMM_WORLD, &req1);
                }
            }
