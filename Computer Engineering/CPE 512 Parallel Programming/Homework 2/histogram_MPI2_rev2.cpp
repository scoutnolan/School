...
To compile on dmc.asc.edu
   ...
   Intel Compiler
      ...
      mpic++ histogram_MPI2.cpp -o histogram_MPI2_intel -std=c++11
...
To execute:
  ...
  On Intel compiler
     use run_script_mpi histogram_MPI2_intel.sh
*/  
...
int main(int argc, char *argv[]) {
   ...
   // Local list size for MPI process & Scatterv calculation
   const int local_list_size = ceil(double(list_size) / double(numprocs)); // Set for ideal case 
   int neps = 0; int offset = 0;                   // Offset and number of entries per section
   int local_array_offset[numprocs] = {};          // Entries = local array offset for each process.
   int numbers_local_array_sizes[numprocs] = {};   // Local array sizes. Use the 2a distribution.
   for(int i = 0; i < numprocs; i++){              // Populate the local array offset with the offset for 
      if(i < list_size % numprocs) neps = list_size/numprocs + 1;
         else neps = list_size / numprocs;         
      numbers_local_array_sizes[i] = neps;   
      offset += neps;                  // Increment offset 
      local_array_offset[i] = offset;  // Populate offset.
   } 
   // Scatter the numbers array to all MPI processes in the system.
   MPI_Scatterv(numbers, numbers_local_array_sizes, local_array_offset, MPI_DOUBLE, numbers_local, local_list_size, MPI_DOUBLE, 0, MPI_COMM_WORLD);
   // create local histogram of the set of numbers associated with the current MPI proces.
   ...
   // summation of each of the local histograms present on each MPI process
   //reduce(histogram,histogram_local,num_bins,0,rank,numprocs);
   MPI_Reduce(&histogram_local, &histogram, num_bins, MPI_INT, MPI_SUM, rank, MPI_COMM_WORLD);
