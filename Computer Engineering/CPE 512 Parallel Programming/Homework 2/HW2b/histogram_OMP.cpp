...
int main(int argc, char *argv[]) {
   ...
   for(int i = 0; i < nthreads; i++){
      thread_data_array[i].localHistogram = new int[num_bins];
   }
   ...
   #pragma omp parallel private(thread_num)
   {
      ...
      #pragma omp parallel for
         for (int i=thread_data_array[thread_num].start;i<=thread_data_array[thread_num].end;i++) {
            double num = numbers[i];
            if (num>=min_val && num < max_val) {
               num -= min_val;
               int bin_num = (int) (num/bin_range);
               thread_data_array[thread_num].localHistogram[bin_num]++;
            }
         }
      
      #pragma omp parallel for
         for(int j = 0; j < num_bins; j++){
            histogram[j] = histogram[j] + thread_data_array[thread_num].localHistogram[j];
         }
   }
