...
void SQLDatabase::UpdateReadyStatus(QString name, QString country)
{
    QSqlQuery q1;
    ...
    QSqlQuery q4;
    q4.prepare("UPDATE results SET country = ? WHERE name = ?");
    q4.addBindValue(country.toUtf8().data());
    q4.addBindValue(name.toUtf8().data());
    if (!q4.exec())
    {
        qDebug() << db.lastError();
        qDebug() << "Error: Unable to create table due to above error";
